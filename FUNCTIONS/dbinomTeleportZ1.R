...
dbinomTeleportZ1 <- nimbleFunction(
  run = function(
...
rbinomTeleportZ1 <- nimbleFunction(
  run = function(
    ...
    if(Alive == 0){
      # temp <- rbinomPPSingle(1,lowerCoords, upperCoords, intensityWeightsRecruit, areAreas, numWindows)
      #  return(temp)
      return(sourceCoords)
    }
    ...
    if(firstAlive == 1){
      ...
      ## SECOND TIME ALIVE OR MORE
      # if(dispersalToggle == 0){
        temp <- rbinomMNormSourcePPSingle(1, lowerCoords, upperCoords, sourceCoords, normSD, intensityWeightsMove, areAreas, numWindows, localEvalParam)
        return(temp)
      # }else{## TELEPORTATION 
      #   temp <- dbinomPPSingle(x,lowerCoords, upperCoords, intensityWeightsTeleport, areAreas, numWindows,log)
      #   return(temp)
...
registerDistributions(list(dbinomTeleportZ1 = list(
  BUGSdist = "dbinomTeleportZ1(lowerCoords, upperCoords, sourceCoords, normSD, intensityWeightsMove, intensityWeightsRecruit, intensityWeightsTeleport , areAreas, numWindows, firstAlive, Alive, localEvalParam)",
  types = c(
    ...
    "areAreas = double(0)", "numWindows = double(0)", "firstAlive = double(0)",
     "Alive = double(0)", "localEvalParam = double(0)" ),
  pqAvail = FALSE
)))
