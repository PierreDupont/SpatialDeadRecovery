...
dbinomTeleport2 <- nimbleFunction( run = function( x = double(1),                               # Coordinate values to calculate the density
                                                  ...
                                                  dipsersalToggle = double(0, default = 0), # A toggle (0/1) If individual performs large range dispersal or not
...
rbinomTeleport2 <- nimbleFunction( run = function(
      ...
      areAreas = double(0, default = 1),           # Flag denoting whether the lower and upper coordinates are areas or transects
      numWindows = double(0, default = -1),        # Number of observation windows (if negative the number of rows in lowerCoords is used to define this value)
      dipsersalToggle = double(0, default = 0),
...
registerDistributions(list(dbinomTeleport2 = list(
   BUGSdist = "dbinomTeleport2(lowerCoords, upperCoords, sourceCoords, normSD, intensityWeights, areAreas, numWindows, dipsersalToggle, localEvalParam, indicator)",
   types = c(
      "value = double(1)", "lowerCoords = double(2)",
      "upperCoords = double(2)", "sourceCoords = double(1)", "normSD = double(0)", 
      "intensityWeights = double(1)", "areAreas = double(0)", "numWindows = double(0)", "dipsersalToggle = double(0)", "localEvalParam = double(0)", "indicator = double(0)" ),
   pqAvail = FALSE
)))