#' @title Function to calculate rescale the coordinates in to UTM from a SpatialPointsDataFrame object based on a another SpatialPointsDataFrame that contains locations of grid cell centers of a regular grid.
#' #'
#' @description
#' \code{GridToUTM} returns a dataframe with x and y coordinates.
#' 
...
#' # UTMToGrid:
#' 
#'GridToUTM(data.sp = detectorscaled.sp, grid.sp = habitat.sp, plot.check = TRUE)
#
#' 
#' 
#'  
  ...
  plot.check = TRUE)
{
  ...
  data.scaled.xy[ ,"x"] <- start0.x + data.xy[ ,"x"] * resolution 
  }
  ...
  if(length(dim(data.xy))==3){
     ...
     data.scaled.xy[ ,,1] <- start0.x + data.xy[ ,,1] * resolution 
  }
  # ITEATIONS,i,xy,t
