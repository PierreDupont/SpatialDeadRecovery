...
#' @param post.repro A \code{logical} if TRUE, sampling occur right after repro. So individuals needs to survive first and then can reproduce.
#'                                    if FALSE, sampling occur right before repro. So individuals reproduces and then surive.
#' 
...
SimulateZ.PopDynLeslie <- function( N0 = NULL
                            , n.occasions
                            , PHI
                            , REPRO
                            , FEC  
   ...
   for(t in 2:(n.occasions+1)){
      ...
      for(i in 1:(dim(POP[[t-1]])[1])){
            ...
            R[i] <- rbinom(1, 1, REPRO[INIT.STATE,Z[i], t-1 ])
            if(R[i]==0){
               B[i] <-  0
            }else{
            B[i] <- rpois(1, FEC[Z[i], t-1 ])
            }
         }#i
      }else{
         for(i in 1:(dim(POP[[t-1]])[1])){
