...
SimulateDetectionLCP <- function( p0 = NULL 
                               , a0 = NULL
                               , sigma
                               , AC.sp
                               , detector.sp
                               ...
                               )
{
   ...
   if(typeD == 3){
      cost <- exp(betas.LCP * raster.LCP)
      #conductance <- transition(cost, transitionFunction=function(x)(1/(mean(x))), direction = 8) ##--symmetric
      conductance <- transition(cost, transitionFunction = function(x) max(x) - x[1] + x[2] , symm=FALSE, directions = 8)
      D <- as.matrix(costDistance(conductance, AC.sp, detector.sp))
      }#if 
