...
#### 1.Density function ####
dSCR_LESS <- nimbleFunction(run = function( x = double(1)
                                       , pZero = double(0)
                                       , sigma = double(0)
                                       , sxy = double(1)
   ...
   for(j in 1:n.detectors){
      ...
      if(d2[j] <= (maxDist*maxDist)){p <- pZero * exp(alpha * d2[j])} else {p <- 0.0}
...
#### 2.Sampling function ####
rSCR_LESS <- nimbleFunction(run = function( n = integer(0)
                                       , pZero = double(0)
                                       , sigma = double(0)
                                       , sxy = double(1)
                                       , detector.xy = double(2)
...
registerDistributions(list(
   dSCR_LESS = list(
      BUGSdist = "dSCR_LESS(pZero, sigma, sxy, detector.xy, maxDist, indicator)",
      types = c( "value = double(1)", "pZero = double(0)", "sigma = double(0)"
                 ,"sxy = double(1)", "detector.xy = double(2)", "maxDist = double(0)"
                 ,"indicator = double(0)"),
      pqAvail = FALSE)))
