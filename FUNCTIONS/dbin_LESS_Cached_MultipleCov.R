...
#### 1.Density function ####
dbin_LESS_Cached_MultipleCov <- nimbleFunction(run = function( x = double(1),
                                                          sxy = double(1),
                                                          p0 = double(1),
                                                          sigma = double(0),
   ...
   for(j in 1:nDetectors){
      if(index1[count] == j){ # IF J IS EQUAL TO THE RELEVANT DETECTOR 
         ...
         pZero <- ilogit(logit(p0[detCounties[j]]) +
...
registerDistributions(list(
   dbin_LESS_Cached_MultipleCov = list(
      BUGSdist = "dbin_LESS_Cached_MultipleCov(sxy, sigma, p0, nbDetections, yDets, 
      detector.xy, trials, detectorIndex, nDetectorsLESS, resizeFactor, maxNBDets,
      habitatID, indicator, detCounties, detCov, betaCov, betaResponse, detResponse)",
      types = c( "value = double(1)","sxy = double(1)", "sigma = double(0)","p0 = double(1)",
                 ...
                 "maxNBDets = double(0)","habitatID = double(2)","indicator = double(0)",
                 "detCounties = double(1)", "detCov = double(2)", "betaCov = double(1)",
