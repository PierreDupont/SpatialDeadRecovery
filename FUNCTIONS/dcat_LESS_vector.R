...
#'
#' @description
#' \code{dcat_LESS_vector} returns the likelihood of a given categorical spatial detection y[i] 
#'
#' @param x \code{Vector} of length n.detectors containing observations/non-observations (dbern)
#'                        or a scalar with the ID of the detector where detections happened (dcat).
#' @param pZero \code{Vector} denoting the baseline detection probability of the half-normal at each detector.
...
#### 1.Density functions ####
dcat_LESS_vector <- nimbleFunction(run = function( x = double(0),
                                                   pZero = double(1),
                                                   sigma = double(0),
                                                   d2 = double(1),
   ...
   for(j in 1:n.detectors){
      if(d2[j] <= (maxDist_squared)){p[j] <- pZero[j] * exp(alpha * d2[j])}
   }#j
...
rcat_LESS_vector <- nimbleFunction(run = function( n = integer(0),
                                                   pZero = double(1),
                                                   sigma = double(0),
                                                   d2 = double(1),
                                                   maxDist = double(0, default = 0.0),
...
registerDistributions(list(
    dcat_LESS_vector = list(
      BUGSdist = "dcat_LESS_vector(pZero, sigma, d2, maxDist, indicator)",
      types = c("value = double(0)", "pZero = double(1)", "sigma = double(0)", "d2 = double(1)", "maxDist = double(0)" ,"indicator = double(0)"),
      pqAvail = FALSE)))
