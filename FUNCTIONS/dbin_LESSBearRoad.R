...
#### 1.Density function ####
dbin_LESSBearRoad <- nimbleFunction(run = function( x = double(1)
                                            , sigma = double(0)
                                            , trials = double(1)
                                            , d2 = double(1)
...
#### 2.Sampling function ####
rbin_LESSBearRoad <- nimbleFunction(run = function( n = integer(0)
                                            , sigma = double(0)
                                            , trials = double(1)
                                            , d2 = double(1)
                                            , maxDist = double(0, default = 0.0)
...
registerDistributions(list(
  dbin_LESSBearRoad = list(
    BUGSdist = "dbin_LESSBearRoad(sigma, trials, d2, maxDist, indicator, z, p0State, idResponse,
    detCountries, detRoads,  betaResponse,  betaRoads)",
    types = c( "value = double(1)", "sigma = double(0)", "trials = double(1)", "d2 = double(1)", 
               "maxDist = double(0)" ,"indicator = double(0)", "z = double(0)", "p0State = double(2)",
                 "idResponse = double(0)", "detCountries = double(1)", "detRoads = double(1)",
                "betaResponse = double(0)" ,  "betaRoads = double(0)" ),
    pqAvail = FALSE)))

# registerDistributions(list(
#    dbin_LESSWolfSnow = list(
#       BUGSdist = "dbin_LESSWolfSnow(sigma, trials, d2, maxDist, indicator, z, p0State, idResponse,
#     detCountries, detRoads, detTracks, detSnow, betaResponse, betaTracks, betaRoads, betaSnow)",
#       types = c( "value = double(1)", "sigma = double(0)", "trials = double(1)", "d2 = double(1)", 
#                  "maxDist = double(0)" ,"indicator = double(0)", "z = double(0)", "p0State = double(2)",
#                  "idResponse = double(0)", "detCountries = double(1)", "detRoads = double(1)","detTracks = double(1)"
#                  ,"detSnow = double(1)", "betaResponse = double(0)" , "betaTracks = double(0)", "betaRoads = double(0)" , "betaSnow = double(0)"),
#       pqAvail = FALSE)))