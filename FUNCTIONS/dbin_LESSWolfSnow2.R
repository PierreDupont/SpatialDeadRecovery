...
#### 1.Density function ####
dbin_LESSWolfSnow2 <- nimbleFunction(run = function( x = double(1)
                                            , sigma = double(0)
                                            , trials = double(1)
                                            , d2 = double(1)
...
#### 2.Sampling function ####
rbin_LESSWolfSnow2 <- nimbleFunction(run = function( n = integer(0)
                                            , sigma = double(0)
                                            , trials = double(1)
                                            , d2 = double(1)
                                            , maxDist = double(0, default = 0.0)
...
registerDistributions(list(
  dbin_LESSWolfSnow2 = list(
    BUGSdist = "dbin_LESSWolfSnow2(sigma, trials, d2, maxDist, indicator, z, p0State, idResponse,
    detCountries, detRoads, detTracks, detSnow, detSnow2, betaResponse, betaTracks, betaRoads, betaSnow, betaSnow2)",
    types = c( "value = double(1)", "sigma = double(0)", "trials = double(1)", "d2 = double(1)", 
               ...
               ,"detSnow = double(1)","detSnow2 = double(1)", "betaResponse = double(0)" , "betaTracks = double(0)",
               "betaRoads = double(0)" , "betaSnow = double(0)", "betaSnow2 = double(0)"),
    pqAvail = FALSE)))
