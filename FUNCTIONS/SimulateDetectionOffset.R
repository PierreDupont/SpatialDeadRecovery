...
SimulateDetectionOffset <- function( p0 
                               , sigma
                               , AC.sp
                               , detector.sp
                               , det.rand = NULL
   ...
   if(type == "Bernoulli" | type == "Binomial"){
      ...
      if(is.null(det.rand))det.rand <- 0
      p0 <- inv.logit(intercept.p0 + id.fixed.effects.p0 + det.fixed.effects.p0 + det.rand)
      if(!is.null(det.offset))p0 <- 1-(1-p0)^det.offset
      }
   if(type == "Poisson"){
