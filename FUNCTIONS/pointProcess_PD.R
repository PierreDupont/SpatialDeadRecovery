# ## pointProcess_PD.R
# ## This file contain's comments from Perry, 15 July 2019
# 
...
#        ## [from PdV:] Possibly alternative implementation:
#        ##  chosenWindow <- -1
#        ## for(i in 1:numObsWindows) {
#        ##   if(check if this is the right window) {
#        ##     chosenWindow <- i
#        ##     break
#        ##   }
#        ## }
#        ## if(chosenWindow == -1) stop("did not find a valid window")
#        ## return(chosenWindow)
...
#        ## [from PdV: Big picture: It would be nice to have obsWindow size cached.  That would mean calculating it outside the model and providing it in constants, then passing it to this function.  Or, it would mean calculating it in the model code, such that it will be calculated only once because its parent nodes (lowerCoords, upperCoords), will not change during MCMC.
#        ## E.g. the model could have something like obsWindowSize[1:numWindows] <- calcObsWindowSize(...), details TBD.
#        ## [from PdV: Small picture: I think one could replace `matrix(lowerCoords[1:numObsWindows, 1:dimCoords], nrow = numObsWindows, ncol = dimCoords)` with simply `lowerCoords[1:numObsWindows, 1:dimCoords]` or simply `lowerCoords`.  The way it is written forces an extra copy, I think.  This may or may not be noticeable in computational cost.  If this was done because lowerCoords and upperCoords are integers and you need doubles . If that is the case, it might be better to simply provide the lowerCoords and upperCoords as doubles (i.e. numeric in R).
...
#        ## [from PdV:] Why is a recycling rule application needed here?  Should intensity weights always match
#        ## numObsWindows in length? Or is this recycling rule to handle also the case that intensity weights is a single number?
#        ## If so there might be other ways to do it, with a couple of if-then-else statements below.
#        ## E.g.  useSingleIntensityWeight <- length(intensityWeights) == 1
#        ## followed by if(useSingleIntensityWeight) {}
...
#        ## [From PdV:] We could consider pulling sumIntensity out into the model.  I think it would only change when
#        ## the intensity weights from the model change.  In the case I am currently looking at, that would only be
#        ## when beta.dens is sampled.
...
#          ## [from PdV:] We could probably make this faster. I believe isWithinWindow returns a vector of all zeros except
#          ## for a single 1.  This is then multiplied component-wise with recIntensityWeights.
#          ## I think instead there could be a function that returns the index of the single non-zero weight.
#          ## Even better would be to have a cached fast-lookup system, similar to what is used in dbin_LESS[etc] functions.
...
#             ## [from PdV:] Possibly small inefficiency due to converting to matrix, but I doubt it is noticeable.
...
#        ## [from PdV:] It would seem possible to use fast window lookup and also cache the set of relevant
#        ## obs windows (by window index).
#        ## Ideally we could avoid making copies of large blocks of the upper coords and lower coords
#        ## and instead iterate over indices of the windows.
...
#       # Recycle the intensity weights to match the number of observation windows
#        ## [from PdV:] It looks like, if there has been meaningful subsetting from obsWindowReduce, that the recIntensityWeights will
#        ## be wrong because they are not also subset in a matching way.
#        recIntensityWeights <- numeric(length = numObsWindows, value = intensityWeights, recycle = TRUE)
#       # Ensure that the intensity weights are valid
#       if(sum(recIntensityWeights < 0.0) > 0) {
#          # Invalid values for the intensity weights: set likelihood to zero
...
#        ## [from PdV:]  For compiled C/C++, the vectorization benefit will not be same as for R.
#        ## It may be better to write a for loop and iterate over each window, if that means one
#        ## can save the work above of creating new matrices of [upper|lower]Coords.
#        ## [from PdV:] Also we could consider pulling out obsWindowNorm[i, t] as a model node.
#        ## When an sxy[i, t] is being sampled by MCMC, the matching obsWindowNorm[i, t]
#        ## needs to be calculated to calculate logProb of sxy[i, t+1], but the obsWindowNorm[i, t-1]
#        ## does not need to be calculated, and if it is in the model this calculation will be avoided.
#        ##
#        ## [from PdV:] possible profiling approach:
#        ## use run.time() in nimbleFunctions.
#        ## e.g. calcMNormSourceIntTime <- run.time({any code, e.g. obsWindowNorm <- calcMNormSourceInt(...)})
#        ## then print("calcMNormSourceIntTime = ", calcMNormSourceIntTime)
#        ## 
#         calcMNormSourceIntTime <- obsWindowNorm <- calcMNormSourceInt(
...
#             curCoords <- x[pointIter, 1:dimCoords]
#             # Retrieve the observation windows which the point falls within
#             ## [from PdV:] It would be great if we could instead use a fast lookup system.
#             isInObsWindow <- isWithinWindow(curCoords,
#                matrix(inLowerCoords[1:numObsWindows, 1:dimCoords], nrow = numObsWindows, ncol = dimCoords),
#                matrix(inUpperCoords[1:numObsWindows, 1:dimCoords], nrow = numObsWindows, ncol = dimCoords),
#                tolDist, areAreas)
