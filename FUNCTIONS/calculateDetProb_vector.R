...
calculateDetProb_vector <- nimbleFunction(run = function( sxy = double(1)
   ...
   if(maxDist > 0){
      for(j in 1:n.detectors){
         if(d2[j] <= (maxDist*maxDist)){p[j] <- pZero[j] * exp(alpha * d2[j])}
      }#j
   }else{
      p[1:n.detectors] <- pZero[1:n.detectors] * exp(alpha * d2[1:n.detectors])
   }
