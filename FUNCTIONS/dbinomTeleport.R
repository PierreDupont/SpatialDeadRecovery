...
registerDistributions(list(dbinomTeleport = list(
   BUGSdist = "dbinomTeleport(lowerCoords, upperCoords, sourceCoords, normSD, intensityWeights, areAreas, numWindows, dipsersalToggle, localEvalParam)",
   types = c(
      "value = double(1)", "lowerCoords = double(2)",
      "upperCoords = double(2)", "sourceCoords = double(1)", "normSD = double(0)",
      "intensityWeights = double(1)", "areAreas = double(0)", "numWindows = double(0)", "dipsersalToggle = double(0)", "localEvalParam = double(0)" ),
   pqAvail = FALSE
)))