...
#### 1.Density function ####
dbin_LESSInProd <- nimbleFunction(run = function( x = double(1)
                                            , sigma = double(0)
                                            , trials = double(1)
                                            , d2 = double(1)
...
#--->[PDV]: suggest using inprod function!
#--->[RB]: chaching/sparce matrices?
  for(j in 1:n.detectors){
     pZero[j] <- ilogit(logit(p0State[detCountries[j], z]) +
                           (Xarray[j,1:nCovsP] %*% betaP[1:nCovsP])[1,1])
  }#j
...
#### 2.Sampling function ####
rbin_LESSInProd <- nimbleFunction(run = function( n = integer(0)
                                            , sigma = double(0)
                                            , trials = double(1)
                                            , d2 = double(1)
                                            , maxDist = double(0, default = 0.0)
...
registerDistributions(list(
  dbin_LESSInProd = list(
    BUGSdist = "dbin_LESSInProd(sigma, trials, d2, maxDist, indicator, z, p0State,
    detCountries, Xarray, betaP, nCovsP )",
    types = c( "value = double(1)", "sigma = double(0)", "trials = double(1)", "d2 = double(1)", 
               "maxDist = double(0)" ,"indicator = double(0)", "z = double(0)", "p0State = double(2)",
                  "detCountries = double(1)", "Xarray = double(2)"
                 , "betaP = double(1)","nCovsP = double(0)"),
    pqAvail = FALSE)))
